...
export class CambiarContrasenaComponent implements OnInit {
    ...
    async changePassword() {
        ...
        for(let i=0;i<newPassword.length;i++){
            if(alf.indexOf(newPassword[i]) >=0){
                aux+=1;
            }
        }
      ...
      if (this.securityForm.invalid) {
        ...
        return;
        } /* else  if(aux < this.rules_value[0].num_carac_espe_serecl){
        this._utilService.addWarningMessage('Recuerde que la contraseña debe contener minimo 2 mayusculas.');
        return;
        } */
