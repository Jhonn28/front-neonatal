...
export class OnceComponent implements OnInit {
  ...
  constructor(private _formBuilder: FormBuilder,
    ...
    this.area_salud=_route.snapshot.data['info'][1];
...
//guarda data
  async saveData() {
    ...
    await this._indicadorService.saveData(data, 'her_encabezado_general', 'ide_heg', 'her_tasa_mortalidad').subscribe(async (res: any) => {
      if(res){
  ...
  setData( formulario: FormGroup){
    this.dataForm.push(this._formBuilder.group({
      total_muertes_htm: [formulario.value.total_muertes],
      total_complicaciones_htm: [formulario.value.total_atendidos],
