...
//guarda data
  async saveData(event: any) {
    ...
    await this._indicadorService.saveData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_b').subscribe(async (res: any) => {
      if(res){
    ...
    codigo.value.forEach(element => {
      if (element.codigo_clinica.length > 0) {
        ...
        formulario.value.forEach(element => {
          ...
          if(aux2+1>10){
            if(element['nc'+Number(i+1)] || element[i+1]){
              continua=true;
            }
          }
          ...
          console.log(cumple,suma,aux,aux2,cumple2);
         /*  if (element[i + 1] == true) {
            cumple++;
          }
          if(element[i+1]!=null){
        ...
        //(cumple == suma || cumple == 10) ? this.numerador += 1 : 0;
        if(cumple==suma || ((cumple2+aux)==10 && !continua)){
          this.numerador += 1;
        }
        i++;
      }
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ...
            no_aplica_hcob:[form['nc'+Number(i+1)]]
          }))
        });
        i++;
      }
