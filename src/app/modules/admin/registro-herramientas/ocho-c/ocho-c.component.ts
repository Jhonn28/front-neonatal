...
//guarda data
  async saveData(event: any) {
    ...
    await this._indicadorService.saveData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_c').subscribe(async (res: any) => {
      if(res){
  ...
  promedio(formulario, codigo) {
    ...
    codigo.value.forEach(element => {
      if (element.codigo_clinica.length > 0) {
        ...
        //(cumple == suma || cumple == 12) ? this.numerador += 1 : 0;
        if(cumple==suma || ((cumple2+aux)==12 && !continua)){
          this.numerador += 1;
        }
        i++;
      }
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ...
            no_aplica_hcoc:[form['nc'+Number(i+1)]]
          }))
        });
        i++;
      }
