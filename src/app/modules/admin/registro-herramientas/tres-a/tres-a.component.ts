...
//guarda data
  async saveData(event: any) {
    ...
    await this._indicadorService.saveData(data, 'her_encabezado_general', 'ide_heg', 'her_atencion_parto').subscribe(async (res: any) => {
      if(res){
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          ...
          this.dataForm.push(this._formBuilder.group({
            ide_indapar: [form.ide_indpren],
            nro_historia_clinica_hpa: [element.codigo_clinica],
            cumple_hpa: [form[i + 1]],
            no_aplica_hpa: [form['nc'+Number(i+1)]]
          }))
        });
        i++;
      }
