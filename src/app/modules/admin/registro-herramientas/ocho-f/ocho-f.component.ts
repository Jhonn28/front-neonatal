...
//guarda data
  async saveData(event: any) {
    ...
    await this._indicadorService.saveData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_f').subscribe(async (res: any) => {
      if(res){
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ...
            no_aplica_hcof:[form['nc'+Number(i+1)]]
          }))
        });
        i++;
      }
