...
@Component({
  ...
  styleUrls: ['./ocho-a.component.scss']
})
export class OchoAComponent implements OnInit {
  datosSucursal;
...
//guarda data
  async saveData(event: any) {
    ...
    await this._indicadorService.saveData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_a').subscribe(async (res: any) => {
      console.log(res);
      if(res){
    ...
    this.indicadores.forEach(element => {
      if (element.value != null) {
        this.insumosForm.push(this._formBuilder.group({
          ...
          text:[element.label,[Validators.maxLength(700)]],
          1: [false],
          2: [false],
          3: [false],
          4: [false],
  ...
  promedio(formulario, codigo) {
    ...
    codigo.value.forEach(element => {
      if (element.codigo_clinica.length > 0) {
        ...
        //(cumple == suma || cumple == 7) ? this.numerador += 1 : 0;
        if(cumple==suma || ((cumple2+aux)==7 && !continua)){
          this.numerador += 1;
        }
        i++;
      }
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ...
            no_aplica_hcoa:[form['nc'+Number(i+1)]]
          }))
        });
        i++;
      }
