...
export class OchoAComponent implements OnInit {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_obstetrica_a', this.selectRow, 'nro_historia_clinica_hcoa asc, ide_indcoa asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcoa) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcoa]
        }))
      }
    });
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcoa];
      hash[current.nro_historia_clinica_hcoa] = true;
      return exists;
    });
    return array;
