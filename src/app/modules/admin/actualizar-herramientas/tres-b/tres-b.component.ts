...
@Component({
  ...
  styleUrls: ['./tres-b.component.scss']
})
export class TresBComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ ide: 'ide_indaten', campo: 'detalle_indaten', tabla: 'ind_atencion_par' });
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indaten;
      delete body[index].nro_historia_clinica_hpab;
      body[index]['ide_hpab'] = +element.ide_hpab;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_atencion_parto_b', 'ide_hpab', 'ide_indaten').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '3b', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_atencion_parto_b', this.selectRow, 'nro_historia_clinica_hpab asc, ide_indaten asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hpab) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hpab]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hpab)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ide_indaten: [form.ide],
            nro_historia_clinica_hpab: [element.codigo_clinica],
            cumple_hpab: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hpab];
      hash[current.nro_historia_clinica_hpab] = true;
      return exists;
    });
    return array;
