...
export class SeisComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ide:'ide_indaren',campo:'detalle_indaren',tabla:'ind_atencion_recien_nacido'});
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indaren;
      delete body[index].nro_historia_clinica_harn;
      body[index]['ide_harn'] = +element.ide_harn;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_atencion_recien_nacido', 'ide_harn', 'ide_indaren').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '6', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_atencion_recien_nacido', this.selectRow, 'nro_historia_clinica_harn asc, ide_indaren asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_harn) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_harn]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_harn)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ide_indaren: [form.ide],
            nro_historia_clinica_harn: [element.codigo_clinica],
            cumple_harn: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_harn];
      hash[current.nro_historia_clinica_harn] = true;
      return exists;
    });
    return array;
