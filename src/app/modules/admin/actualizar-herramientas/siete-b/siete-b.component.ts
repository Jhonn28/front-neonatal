...
export class SieteBComponent implements OnInit {
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      body[index]['ide_hapfb'] = +element.ide_hapfb;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_atencion_profesional_b', 'ide_hapfb', 'ide_indaten').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '7b', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_atencion_profesional_b', this.selectRow, 'nro_historia_clinica_hapfb asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hapfb) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hapfb]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hapfb)
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hapfb];
      hash[current.nro_historia_clinica_hapfb] = true;
      return exists;
    });
    return array;
  }
