...
export class OnceComponent implements OnInit {
  ...
  async saveData(event) {
    ...
    this.dataForm.push(this._formBuilder.group({
      ide_htm:[this.datosIndicador[0].ide_htm],
      total_muertes_htm: [t_muertes],
      total_complicaciones_htm: [t_atendidos],
      porcentaje_htm: [porcentaje],
    }))
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_tasa_mortalidad', 'ide_htm', 'indpospar').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '11', query).subscribe(resp => {
  ...
  async loadData() {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_tasa_mortalidad', this.selectRow, 'ide_htm asc');
    ...
    this.herramientasForm.get('total_muertes').setValue(this.datosIndicador[0].total_muertes_htm);
    this.herramientasForm.get('total_atendidos').setValue(this.datosIndicador[0].total_complicaciones_htm);
    this.herramientasForm.get('porcentaje').setValue(this.datosIndicador[0].porcentaje_htm);
