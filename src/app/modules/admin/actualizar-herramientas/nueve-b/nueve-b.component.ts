...
@Component({
  selector: 'app-nueve-b',
  templateUrl: './nueve-b.component.html',
  styleUrls: ['./nueve-b.component.scss']
})
export class NueveBComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ide:'ide_indcnb',campo:'detalle_indcnb',tabla:'ind_complicacion_neonatal_b'});
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indcnb;
      delete body[index].nro_historia_clinica_hcnb;
      body[index]['ide_hcnb'] = +element.ide_hcnb;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_neonatal_b', 'ide_hcnb', 'ide_indcnb').subscribe(async (res: any) => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_neonatal_b', this.selectRow, 'nro_historia_clinica_hcnb asc, ide_indcnb asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcnb) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcnb]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hcnb)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ide_indcnb: [form.ide],
            nro_historia_clinica_hcnb: [element.codigo_clinica],
            cumple_hcnb: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcnb];
      hash[current.nro_historia_clinica_hcnb] = true;
      return exists;
    });
    return array;
  }
