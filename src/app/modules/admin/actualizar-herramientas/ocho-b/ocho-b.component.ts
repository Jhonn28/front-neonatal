...
@Component({
  ...
  styleUrls: ['./ocho-b.component.scss']
})
export class OchoBComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ide:'ide_indcob',campo:'detalle_indcob',tabla:'ind_complicacion_obstetrica_b'});
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indcob;
      delete body[index].nro_historia_clinica_hcob;
      body[index]['ide_hcob'] = +element.ide_hcob;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_b', 'ide_hcob', 'ide_indcob').subscribe(async (res: any) => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_obstetrica_b', this.selectRow, 'nro_historia_clinica_hcob asc, ide_indcob asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcob) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcob]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hcob)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ide_indcob: [form.ide],
            nro_historia_clinica_hcob: [element.codigo_clinica],
            cumple_hcob: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcob];
      hash[current.nro_historia_clinica_hcob] = true;
      return exists;
    });
    return array;
