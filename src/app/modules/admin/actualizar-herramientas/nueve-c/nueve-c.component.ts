...
export class NueveCComponent implements OnInit {
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indcnc;
      delete body[index].nro_historia_clinica_hcnc;
      body[index]['ide_hcnc'] = +element.ide_hcnc;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_neonatal_c', 'ide_hcnc', 'ide_indcnc').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '9c', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_neonatal_c', this.selectRow, 'nro_historia_clinica_hcnc asc, ide_indcnc asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcnc) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcnc]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hcnc)
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcnc];
      hash[current.nro_historia_clinica_hcnc] = true;
      return exists;
    });
    return array;
  }
