...
@Component({
  ...
  styleUrls: ['./cuatro.component.scss']
})
export class CuatroComponent implements OnInit {
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      delete body[index].nro_historia_clinica_hpac;
      body[index]['ide_hpac'] = +element.ide_hpac;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_atencion_parto_c', 'ide_hpac', 'ide_indaten').subscribe(async (res: any) => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_atencion_parto_c', this.selectRow, 'nro_historia_clinica_hpac asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hpac) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hpac]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hpac)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            nro_historia_clinica_hpac: [element.codigo_clinica],
            cumple_hpac: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hpac];
      hash[current.nro_historia_clinica_hpac] = true;
      return exists;
    });
    return array;
  }
