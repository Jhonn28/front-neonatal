...
@Component({
  ...
  styleUrls: ['./ocho-d.component.scss']
})
export class OchoDComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ide:'ide_indcod',campo:'detalle_indcod',tabla:'ind_complicacion_obstetrica_d'});
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indcod;
      delete body[index].nro_historia_clinica_hcod;
      body[index]['ide_hcod'] = +element.ide_hcod;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_d', 'ide_hcod', 'ide_indcod').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '8d', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_obstetrica_d', this.selectRow, 'nro_historia_clinica_hcod asc, ide_indcod asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcod) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcod]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hcod)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ide_indcod: [form.ide],
            nro_historia_clinica_hcod: [element.codigo_clinica],
            cumple_hcod: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcod];
      hash[current.nro_historia_clinica_hcod] = true;
      return exists;
    });
    return array;
