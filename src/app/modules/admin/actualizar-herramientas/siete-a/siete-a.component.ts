...
@Component({
  selector: 'app-siete-a',
  templateUrl: './siete-a.component.html',
  styleUrls: ['./siete-a.component.scss']
})
export class SieteAComponent implements OnInit {
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      body[index]['ide_hapf'] = +element.ide_hapf;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_atencion_profesional', 'ide_hapf', 'ide_indaten').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '7a', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_atencion_profesional', this.selectRow, 'nro_historia_clinica_hapf asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hapf) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hapf]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hapf)

      index_indicador++;

      if (index_indicador == 1) {
        index_indicador = 0;
        index_codigo++;
      }
    });
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            nro_historia_clinica_hapf: [element.codigo_clinica],
            cumple_hapf: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hapf];
      hash[current.nro_historia_clinica_hapf] = true;
      return exists;
    });
    return array;
  }
