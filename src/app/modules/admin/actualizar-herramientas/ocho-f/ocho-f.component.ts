...
@Component({
  ...
  styleUrls: ['./ocho-f.component.scss']
})
export class OchoFComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ide:'ide_indcof',campo:'detalle_indcof',tabla:'ind_complicacion_obstetrica_f'});
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indcof;
      delete body[index].nro_historia_clinica_hcof;
      body[index]['ide_hcof'] = +element.ide_hcof;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_obstetrica_f', 'ide_hcof', 'ide_indcof').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '8f', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_obstetrica_f', this.selectRow, 'nro_historia_clinica_hcof asc, ide_indcof asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcof) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcof]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hcof)
      ...
      if (element.codigo_clinica.length > 0) {
        formulario.value.forEach(form => {
          this.dataForm.push(this._formBuilder.group({
            ide_indcof: [form.ide],
            nro_historia_clinica_hcof: [element.codigo_clinica],
            cumple_hcof: [form[i + 1]],
          }))
        });
        i++;
      }
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcof];
      hash[current.nro_historia_clinica_hcof] = true;
      return exists;
    });
    return array;
