...
@Component({
  selector: 'app-nueve-a',
  templateUrl: './nueve-a.component.html',
  styleUrls: ['./nueve-a.component.scss']
})
export class NueveAComponent implements OnInit {
  ...
  async ngOnInit() {
    this.indicadores = await this._indicadorService.getIndicador({ide:'ide_indcna',campo:'detalle_indcna',tabla:'ind_complicacion_neonatal_a'});
  }
  ...
  async saveData(event) {
    ...
    this.datosIndicador.forEach(element => {
      ...
      delete body[index].ide_indcna;
      delete body[index].nro_historia_clinica_hcna;
      body[index]['ide_hcna'] = +element.ide_hcna;
      index++;
    });
    ...
    await this._indicadorService.updateData(data, 'her_encabezado_general', 'ide_heg', 'her_complicacion_neonatal_a', 'ide_hcna', 'ide_indcna').subscribe(async (res: any) => {
  ...
  async buscarSupervision() {
    ...
    await this._indicadorService.getEncabezadoGeneral(distrito, '9a', query).subscribe(resp => {
  ...
  async loadData(ide: number) {
    this.datosIndicador = await this._indicadorService.getDataIndicador('her_complicacion_neonatal_a', this.selectRow, 'nro_historia_clinica_hcna asc, ide_indcna asc');
    //Carga indicadores
    ...
    codigo.forEach(element => {
      if (element.nro_historia_clinica_hcna) {
        this.codigoForm.push(this._formBuilder.group({
          codigo_clinica: [element.nro_historia_clinica_hcna]
        }))
      }
    });
    ...
    this.datosIndicador.forEach(element => {
      ...
      form[index_indicador].get(String(index_codigo)).setValue(element.cumple_hcna)
  ...
  filterCodigo(array) {
    ...
    array = array.filter(function (current) {
      let exists = !hash[current.nro_historia_clinica_hcna];
      hash[current.nro_historia_clinica_hcna] = true;
      return exists;
    });
    return array;
  }
