  ...
  validateToken(): Observable<boolean> {
    ...
    return this._methodService.get(`auth/renew-token/${user}`).pipe(
      map((resp: IResultAuth) => {
        const { ide_segusu, nombre_segusu, username_segusu, correo_segusu, tema_segusu, nombre_segper, foto_segusu } = resp.user[0];
        this.usuario = new UserCore(ide_segusu, nombre_segusu, username_segusu, correo_segusu, nombre_segper, tema_segusu, foto_segusu);
